...
/**
 * v6 SEAL-PREDICTION (擊殺預判). A foe with >= SEAL_FREE_CANNONS_MIN spare bombs
 * can ADVANCE and drop a 2-3 bomb vChain to wall the bot in — the death that
 * dominates v6's losses (v5-diag: TRAPPED 31/42, escape branches collapse ~10 s
 * before death). The depth-4 search and entrapWeight model the foe at its CURRENT
 * tile / a single bomb, so they miss the advancing multi-bomb cap. Seal-prediction
 * is a SEPARATE, longer-range, higher-branch-target penalty gated on the foe
 * actually HOLDING the cannons to chain — it biases the bot to open ground BEFORE
 * the seal forms. It complements entrapWeight (kept cannon-INDEPENDENT for the
 * mid-seal phase where the foe's cannons are already spent — see §九 lesson).
 */
const SEAL_PREDICT_RANGE = 8; // Manhattan: just beyond combat range (~7) — the seal-setup band.
const SEAL_BRANCH_TARGET = 2; // dead-end definition (same as entrap); the extra reach is the value.
const SEAL_FREE_CANNONS_MIN = 3; // only a REAL chainer (>=3 spare bombs) — fires rarely, not constantly.
...
export class BotController {
  ...
  /** POINT 2 option C: hard center-priority weight ONCE the shrink is active
   * (per-map `MapProfile.shrinkCenterPriorityWeight`); 0 = off. Read by
   * leafReward — overrides curShrinkWeight while the shrink is live. */
  private curShrinkCenterPriorityWeight = 0;
  ...
  /** v6 SHRINK escape-branch weight for THIS decision (per-map
   * `MapProfile.shrinkEntrapWeight`); 0 = off. Foe-INDEPENDENT dead-end penalty
   * that fires during the sudden-death lead-in (the wall is the sealer), scaled by
   * shrink proximity. Read by leafReward. */
  private curShrinkEntrapWeight = 0;

  /** v6 SEAL-PREDICTION weight for THIS decision (per-map
   * `MapProfile.sealPredictWeight`); 0 = off. Read by leafReward to penalise low-
   * branch (sealable) tiles when a foe holding >= SEAL_FREE_CANNONS_MIN spare bombs
   * is within SEAL_PREDICT_RANGE — anticipating its advancing vChain seal. */
  private curSealPredictWeight = 0;

  /** Free cannons (cannon - activeBombs) of the nearest attackable foe THIS
   * decision; 0 when none. Read by leafReward to gate seal-prediction (a foe needs
   * >= SEAL_FREE_CANNONS_MIN spare bombs to chain a seal). */
  private curFoeFreeCannons = 0;
  ...
  private leafReward(
    ...
    // Shrink proximity ramp (0..100), shared by the center-rank pull AND the v6
    // shrink escape-branch defense below: 0 until SHRINK_LEAD_TICKS before the
    // sudden-death start, then ramps linearly to 100 at the start and holds.
    const shrinkProx =
      state.tick <= SUDDEN_DEATH_START_TICK - SHRINK_LEAD_TICKS
        ...
        : Math.min(
            100,
            Math.floor(
              ((state.tick - (SUDDEN_DEATH_START_TICK - SHRINK_LEAD_TICKS)) * 100) /
                SHRINK_LEAD_TICKS,
            ),
          );
    // POINT 2 option C「縮圈開始之後，佔據中心」: once the shrink is ACTUALLY live
    // (tick ≥ start), if the per-map hard-priority weight is set, the center-
    // survival term switches to that (large) weight so grabbing/holding the late-
    // hardening center dominates the hunt/seal pulls. Before the shrink starts
    // (lead-in) the soft curShrinkWeight still governs the drift. 0 = off → soft
    // pull throughout.
    const shrinkActive = state.tick >= SUDDEN_DEATH_START_TICK;
    const effShrinkWeight =
      this.curShrinkCenterPriorityWeight > 0 && shrinkActive
        ? this.curShrinkCenterPriorityWeight
        : this.curShrinkWeight;
    let shrinkScaled = 0;
    if (effShrinkWeight > 0 && shrinkProx > 0) {
      shrinkScaled = Math.floor(
        (effShrinkWeight * SHRINK_SURVIVAL_RANK[idx(rx, ry)]! * shrinkProx) / 100,
      );
    }
    ...
    // v6 SEAL-PREDICTION (擊殺預判): a foe HOLDING >= SEAL_FREE_CANNONS_MIN spare
    // bombs within SEAL_PREDICT_RANGE can advance and chain a 2-3 bomb vChain to wall
    // us in — the TRAPPED death that dominates v6's losses (v5-diag: 31/42, branches
    // collapse ~10 s before death). entrapWeight (cannon-INDEPENDENT, range ~7,
    // target 2) catches the IMMINENT seal; this catches the SETUP — longer range and
    // a stricter SEAL_BRANCH_TARGET (3, since a chainer can cap 2 exits), gated on the
    // foe actually having the cannons to chain — so the bot drifts to open ground
    // BEFORE the seal forms. Folds into entrapPenalty (same sign/shape). The hard
    // refuge gate / survivability flood are untouched — it only re-weights safe tiles.
    if (
      this.curSealPredictWeight > 0 &&
      this.curFoeFreeCannons >= SEAL_FREE_CANNONS_MIN &&
      foeTileIdx >= 0
    ) {
      const fx = foeTileIdx % MAP_COLS;
      const fy = (foeTileIdx - fx) / MAP_COLS;
      const man = Math.abs(rx - fx) + Math.abs(ry - fy);
      if (man <= SEAL_PREDICT_RANGE) {
        const deficit = Math.max(
          0,
          SEAL_BRANCH_TARGET - this.escapeBranches(state, danger, rx, ry),
        );
        if (deficit > 0) {
          const prox = Math.max(0, SEAL_PREDICT_RANGE + 1 - man); // closer chainer → bigger.
          entrapPenalty += this.curSealPredictWeight * deficit * prox;
        }
      }
    }
    // v6 SHRINK ESCAPE-BRANCH defense: during the sudden-death lead-in the WALL is
    // the sealer (foe-independent). v5's anti-entrapment only fires with a foe in
    // combat range, so it is OFF for the ~20/38 mirror losses where the shrink
    // squeezes the bot into a dead-end pocket with the foe FAR (v5-diag: death
    // foeDist 6.66, enemyBombsNear ~0.5, branches 1.76 → 0.29). Penalise low-branch
    // result tiles by shrink proximity so the bot keeps an exit as the arena hardens
    // — biased in EARLY (the collapse is visible ~10 s out but only ~3 s of
    // survivability look-ahead sees it, which is why reacting late didn't help). Foe-
    // independent; the hard refuge gate / survivability flood are untouched, so it
    // only re-weights WHICH safe tile is preferred, never overrides safety.
    if (this.curShrinkEntrapWeight > 0 && shrinkProx > 0) {
      const deficit = Math.max(
        0,
        ENTRAP_BRANCH_TARGET - this.escapeBranches(state, danger, rx, ry),
      );
      if (deficit > 0) {
        entrapPenalty += Math.floor(
          (this.curShrinkEntrapWeight * deficit * shrinkProx) / 100,
        );
      }
    }
    return (
  ...
  sample(state: SimState, slot: number): InputFrame {
    ...
    this.curShrinkCenterPriorityWeight = profile.shrinkCenterPriorityWeight;
    this.curShrinkEntrapWeight = profile.shrinkEntrapWeight;
    this.curSealPredictWeight = this.tuning.sealPredictWeight ?? 0;
    ...
    // v6 SEAL-PREDICTION: spare bombs the nearest foe can chain into a vChain seal
    // (cannon - activeBombs). Captured here from the same foe lookup; read by
    // leafReward to gate the seal-prediction penalty. 0 when no foe is reachable.
    this.curFoeFreeCannons = 0;
    if (nearestFoeTileIdx >= 0) {
      for (const p of state.players) {
        ...
        if (idx(tileOf(p.posX), tileOf(p.posY)) === nearestFoeTileIdx) {
          ...
          this.curFoeFreeCannons = Math.max(0, p.cannon - p.activeBombs);
          break;
        }
      }
    }
    ...
    // POINT 1「聯通之前不要停止發育」(v8, from v7): while genuinely ISOLATED (no open
    // path to any foe), do NOT fade the development floor with clock urgency —
    // keep developing to completion until an open path to a foe exists. You cannot
    // hunt a foe you are walled off from, and sudden-death is guaranteed to force
    // the connection later, so the old urgency fade (× (100−urgency)/100, a farm-
    // to-timeout guard) is vestigial for the isolated case and made the bot stop
    // developing while still cut off. Once connected (isolated=false) the normal
    // readiness / kill doctrine governs as before.
    const isolatedFloor = profile.isolatedDevFloor;
    ...
    // Goal #3 STRIP-THE-FOE'S-SURROUNDINGS ("把他周遭都先炸光"): while NOT in close
    // combat (isolated OR connected-but-not-engaged), bias the farm target toward
    // bricks NEAR the nearest foe (by MANHATTAN), so the bot clears the foe's
    // neighbourhood into OPEN space — stripping its cover while keeping many escape
    // routes (an area, not a single sealable corridor; the defensive stack keeps the
    // bot out of dead-ends throughout). Off (digFoeX < 0) when engaged or weight 0.
    const digToFoeWeight = this.tuning.digToFoeWeight ?? 0;
    let digFoeX = -1;
    let digFoeY = -1;
    if (digToFoeWeight > 0 && !foeEngaged && foeTilesNow.size > 0) {
      let bestMan = Number.MAX_SAFE_INTEGER;
      for (const fIdx of foeTilesNow) {
        const fx = fIdx % MAP_COLS;
        const fy = (fIdx - fx) / MAP_COLS;
        const m = Math.abs(fx - myX) + Math.abs(fy - myY);
        if (m < bestMan) {
          bestMan = m;
          digFoeX = fx;
          digFoeY = fy;
    ...
    let farmScore = Number.MAX_SAFE_INTEGER;
    ...
    for (const [tIdx, info] of reachable) {
      ...
      // Default key = hop distance (nearest brick). When digging, add a foe-ward
      // term so a brick toward the foe can win over a marginally-nearer one; dist is
      // scaled ×4 so the bias detours at most ~weight/4 hops per Manhattan tile saved.
      const score =
        digFoeX >= 0
          ? info.dist * 4 +
            digToFoeWeight * (Math.abs(tx - digFoeX) + Math.abs(ty - digFoeY))
          : info.dist;
      if (score < farmScore) {
        farmScore = score;
        farmDist = info.dist;

...
export interface MapProfile {
  ...
  /**
   * POINT 2「縮圈開始之後，佔據中心」(v8, from v7) — HARD center-priority ONCE the
   * shrink is ACTIVE (tick ≥ SUDDEN_DEATH_START_TICK). 0 = off (use the soft
   * `shrinkSurvivalWeight` pull only). When > 0, the moment the shrink actually
   * starts the center-survival term switches to THIS (large) weight, so reaching
   * / holding the late-hardening center DOMINATES the hunt/seal pulls — the bot
   * commits to grabbing center instead of merely leaning toward it. Still gated
   * by the hard refuge/safety net (only biases WHICH safe tile). A/B-validated
   * before enabling; a value ~10× shrinkSurvivalWeight is "decisively dominant".
   */
  readonly shrinkCenterPriorityWeight: number;
  /**
  ...
  /**
   * v6 SHRINK ESCAPE-BRANCH weight (0 = off, the v5 neutral). A foe-INDEPENDENT
   * dead-end penalty active during the sudden-death lead-in: each result tile with
   * escapeBranches < ENTRAP_BRANCH_TARGET is penalised by
   *   floor(weight × deficit × shrinkProx / 100).
   * Rationale (v5-diag, classic mirror): ~20/38 mirror losses are the shrink wall
   * squeezing the bot into a dead-end pocket with the FOE FAR (foeDist 6.66,
   * enemyBombsNear ~0.5) — outside the range where v5's foe-triggered entrapWeight
   * fires, so v5 has no defence there. This keeps an exit as the arena hardens,
   * complementing shrinkSurvivalWeight (which pulls toward the late-hardening CENTER
   * but ignores whether that tile is a dead-end). CLASSIC on, PIRATE off (the open

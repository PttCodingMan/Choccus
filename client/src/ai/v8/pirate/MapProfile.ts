...
export const PIRATE_PROFILE: MapProfile = Object.freeze({
  ...
  // POINT 2「縮圈開始之後，佔據中心」option C: OFF on pirate. A/B (v5-screen, v7:zoner
  // vs v4:zoner) was consistently NEGATIVE here (−5.6% @w50, −7.3% @w20): pirate
  // already has the strongest SOFT center pull (shrinkSurvivalWeight 6, its tuned
  // mirror-breaker), so a hard center-priority over-centralizes and breaks the
  // tuned endgame. The soft pull alone is the validated pirate endgame.
  shrinkCenterPriorityWeight: 0,
  ...
  farmCadence: true,
  // v6 SHRINK ESCAPE-BRANCH defense: OFF on pirate. TESTED at 30 (the lever §十's
  // center-rank survival never tried here): CRN-paired it's NEUTRAL — 50.4% vs v4,
  // 50.0% vs v5 (520 games each), within noise. Pirate is a ~50% coin-flip for all

...
export class BotController {
  ...
  private leafReward(
    ...
    // POINT 2 option C: once the shrink is ACTUALLY live (tick ≥ start), if the
    // per-map hard-priority weight is set, the center-survival term switches to
    // that (large) weight so grabbing/holding the late-hardening center dominates
    // the hunt/seal pulls. Before the shrink starts (lead-in) the soft
    // curShrinkWeight still governs the drift. 0 = off → soft pull throughout.
  ...
  sample(state: SimState, slot: number): InputFrame {
    ...
    // POINT 1 (v7, 2026-06-28): while genuinely ISOLATED (no open path to any
    // foe), do NOT fade the development floor with clock urgency — keep
    // developing to completion until an open path to a foe exists. You cannot
    // hunt a foe you are walled off from, and sudden-death is guaranteed to
    // force the connection later, so the old urgency fade (× (100−urgency)/100,
    // a farm-to-timeout guard) is vestigial for the isolated case and made the
    // bot stop developing while still cut off. Once connected (isolated=false)
    // the normal readiness / kill doctrine governs as before.

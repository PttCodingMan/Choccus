...
export const CLASSIC_PROFILE: MapProfile = Object.freeze({
  ...
  // POINT 2 option C: ON (weight 20) — hard center-priority once the shrink is
  // active. A/B (v5-screen, v7:zoner vs v4:zoner) was a clear WIN on VILLAGE
  // (+9.4%, z+1.8; village reuses this classic profile) and mild-positive on
  // classic itself (+2.1%). Weight 20 beats 50 (fully-dominant over-centralizes:
  // village only +3.1% @w50). Pirate has its OWN profile with this OFF (there it
  // over-centralizes the already-tuned soft pull). Final value pending the full
  // bt-seed + meta-rank re-validation.
  shrinkCenterPriorityWeight: 20,
  ...
  entrapWeight: 20, // v6: stronger foe-triggered escape redundancy

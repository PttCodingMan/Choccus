/**
 * Strategies — the v7 YARDSTICK ROSTER: a deliberately NON-TRANSITIVE set of
 * archetypes so the 1v1 win-rate MATRIX (not a single Bradley-Terry scalar)
 * reveals the rock-paper-scissors structure. This is the v3 roster ported
 * VERBATIM onto the current engine (v7's BotController/BotConfig/MapProfile are
 * byte-for-byte v6) and FROZEN as the rebuilt BT / α-Rank / Nash-averaging
 * reference field — see version.ts and docs/ai-versions.md §七. Each archetype is
 * built from the shared `BotTuning` knobs (see BotConfig.ts) plus one behaviour-
 * axis flag, so they occupy genuinely different corners of the strategy space
 * while all staying fully deterministic / lockstep-safe.
 *
 ...
 *                      is overfit hot air.
 *
 * Note: the trapper's vChain* knobs are carried verbatim from v3 but are INERT
 * on this engine (the BotController never reads them — true in v3 too); the
 * trapper is differentiated by its aggression / combat-range / mistake tuning,
 * NOT a distinct vChain code path. Kept to preserve the v3 archetype definition.
 *

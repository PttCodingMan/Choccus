...
- **量尺定位 `bt-rank --target=v8:zoner --map=<m> --no-write`（三圖、80×2）**：`v8:zoner` ≡ `v7:zoner`（引擎逐位元、
  zoner 旋鈕逐位元、classic/pirate profile 數值逐位元）⇒ 必回到三圖 #1（與 v7:zoner 並列）；確認跑被容器重啟中斷，
  重跑中。另跑 `v5-probe --target=v8:zoner --opponents=v6:hunter` 量新冠軍 vs 舊冠軍 head-to-head（補上）。

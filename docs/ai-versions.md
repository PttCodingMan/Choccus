...
## v3 目標（`/goal`）：對 v2 最強策略勝率達 80% — ✅ 已達成

| 地圖 | v3 冠軍 | 對該圖最強 v2（皆 aggressor）勝率 | 門檻 |
| --- | --- | --- | --- |
| **classic**（封閉） | **v3-Aggressor** | **81.7%**（對 v2-chaosv 82.8%） | ✅ ≥80% |
| **pirate**（開放） | **v3-Tempering** | **80.8%**（v3-Aggressor 80.3%） | ✅ ≥80% |

> 高樣本確認：`v3-bench --repeats=150`（每格 300 場、約 4,800 場、CRN、平手算 0.5）。
> 「各圖最強 v2」由 `v2-rank`（v2 內部 1v1 循環賽）判定——**兩圖最強 v2 都是 aggressor**
> （classic 56% > chaosv/turtle 51.5%）。注意：舊文件「classic 冠軍＝v2-chaosv」是 v1+v2
> 8-agent 混合矩陣的 rank-1，不是 goal 所指的「v2 內部最強策略」。
>
> v3 在兩圖都**全面壓過 v2**（每個 v3 archetype 都打贏每個 v2 archetype）。第六節為 v3 說明。
...
v3 從 v2 原封複製後演進（v2 凍結不動）。目標：對 v2 最強策略勝率 ≥80%。核心是**連通性教條**＋幾個關鍵修正，最終兩圖達標（classic v3-Aggressor 81.7%、pirate v3-Tempering 80.8%，對最強 v2＝aggressor）。

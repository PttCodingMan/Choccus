...
### live 冠軍仍取進攻的 `v8:hunter`
延續 §十一 使用者定版（玩家要對到**非常積極進攻**的 bot）：`mapChampions` 三圖＝`v8:hunter`＝v6:hunter
進攻前臉（aggression 1.5／bombChance 0.9／combatRange 7／`digToFoeWeight 3`／`sealPredictWeight 12`）＋兩條新規則。
純防守 `?strategy=zoner` 仍可選。難度 easy/medium/hard ＝同一隻進攻 hunter 的三段銳利度（`v8/BotConfig.ts`，逐位元＝v6）。

### 出貨驗證（ship gate ＝對前冠軍 `v6:hunter` 直接 CRN 三圖 ≥50%）
- **lint**（含 `sim/**`＋`ai/**` 決定性護欄）✅；**build**（tsc + vite）✅。
- **`npm run test:ci`**（決定性／banned-token／hit-coverage／matrix CRN…）240 過 ✅。
- **自陷護欄**（`ai-selfkill`／`ai-chaosv`／`ai-live-selftrap`，**import 釘 v6→v8**＝實際守 live 冠軍；`src/selfkill.ts`
  亦由 stale 的 v5 釘正到 v8）：<RESULT 待補>。
- **ship gate `v5-probe --target=v8:hunter --opponents=v6:hunter`（三圖、60×2）**：<RESULT 待補>。
- **量尺定位 `bt-rank --target=v8:zoner --map=<m> --no-write`（三圖、80×2）**：<RESULT 待補>。

### 如何用 / 重跑（同 §十二，target 改 v8）
- ship gate：`npm run v5-probe -- --target=v8:hunter --opponents=v6:hunter`（三圖直接 CRN，是真正出貨判準）。
- 量尺定位：`npm run bt-rank -- --target=v8:zoner --map=<m> --no-write`（對 v7 池；v8:zoner ≡ v7:zoner ⇒ #1）。

...
- **v8**（**最新 / live**，`AI_VERSION = 8`，`client/src/ai/v8/`）＝v6 冠軍 roster ＋ 採用 v7 引擎驗證過的**兩條 BnB 三圖戰術規則**：①**「聯通之前不要停止發育」**（孤立時發育 floor 不再隨時鐘 urgency 衰減——打不到被牆隔開的敵人，縮圈遲早逼出聯通，故孤立就發育到完成；`BotController` `isolatedFloor`）；②**「縮圈開始之後，佔據中心」**（`MapProfile.shrinkCenterPriorityWeight`：縮圈真正啟動後中心存活拉力切到大權重、壓過獵殺/封殺，classic+village 20、pirate 0；仍受硬保命閘約束）。兩者皆為 v6→v7 引擎 delta，v8 其餘逐位元＝v6 冠軍 roster。`v8:zoner` 逐位元＝量尺 #1 的 `v7:zoner` → 三圖 BT 皆 #1，且**真正吃到兩規則**（會發育＋會佔中心）＝**live 冠軍**。進攻 `v8:hunter` 是 `pureHunt`（規則1對它無效）、ship-gate 規則2在 classic 害它退步（classic 46.3% / pirate 50.0% / village 65.8%）→ 不當冠軍，僅 `?strategy=hunter` 可選。詳見 `docs/ai-versions.md` §十三。

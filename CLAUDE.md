...
2. **時間 / 網路 / 渲染（`client/src/{net,render,input,ui,config}`）** — 所有 wall-clock timing 住這層。**Solo / spectate / 離線 bot 房與線上房共用同一個 `net/LockstepEngine.ts`**：差別只在 transport（`net/Transport.ts` 介面）。線上走 `NetClient`（WS relay）；本機走 `net/LoopbackTransport.ts`（無 socket、本地輸入即時 echo）。引擎一律 lockstep：本地輸入排到 `T + INPUT_DELAY_TICKS`（=2，**solo 也一樣**）、（線上）送 relay echo 回**權威版本**，所有 slot 輸入到齊才推進一 tick，每 N tick 比對 `stateHash` 偵測 desync。loopback 永不 stall，故引擎 `update()` 有 per-frame tick cap 防慢機 spiral。統一 runner＝`net/MatchRunner.ts`（rAF 迴圈、倒數、auto-restart、loss recorder）；`main.ts` solo 只建 LoopbackTransport + 本地 `MatchStart` 餵它。**隊伍＝逐人手動指定**（室長點任一玩家/bot 切隊，非室長只能切自己；team 經 `SET_PLAYER_TEAM` → relay 權威 → `MatchStart.teams`）；已無 ffa/2v2 format preset。`render/` 只在最近兩個 state 間內插。
...
**Server relay（`server/`）**：`main.py`＝dev relay（純 ws）、`serve.py`＝production（HTTP static + ws）。`relay/`：`RelayServer` / `TickCoordinator`（收齊輸入才放行該 tick；斷線 slot 補 neutral ghost）/ `Lobby` / `Room`。wire = 1-byte `MsgType` + MessagePack（id 定義在 `shared/protocol.ts`，Python 端手動鏡像）。relay **只中繼輸入、不跑 sim**，但它是房間設定的**權威**：host-only `SET_ROOM_SETTINGS`（map）＋ `SET_PLAYER_TEAM`（room.py 存逐 slot teams、室長改任何人/玩家改自己）→ 開局時塞進 `MatchStart{map,teams}`。`REPLAY_UPLOAD`（client 輸掉時上傳）由 `relay/replays.py` 存成 `npm run replay` 可重跑的檔（有大小上限防濫用）。

**AI 版本制（`client/src/ai/`）**：每個 `ai/vN/` 是獨立、可並存的決策碼快照——**版本本身就是持久化機制，無另存 frozen baseline**（`baselines/` 已移除）。一律透過 `ai/index.ts` 的 version-agnostic factory 取用，呼叫端（`main.ts`、sim-runner）**絕不**直接 import 某 version 資料夾。每圖預設 bot ＝`ai/mapChampions.ts`；2026-06-28 起**全圖 = v8:zoner**（v6 冠軍 roster ＋ 兩條 BnB 三圖戰術規則：①「聯通之前不要停止發育」②「縮圈開始之後，佔據中心」，由 v7 量尺引擎驗證後移植到冠軍線）。**為何取控場 zoner 當冠軍**：這兩條是「發育＋控場」教條，只有會發育的 zoner 真正吃得到（規則1「發育到聯通」＋規則2「縮圈佔中心」皆生效）；進攻 `hunter` 是 `pureHunt` 永不發育→規則1對它無效，且 ship-gate 實測規則2在 classic 害它退步（v8:hunter vs v6:hunter：classic 46.3% / pirate 50.0% / village 65.8%，未過 ≥50% 門檻）。故**三圖第一且真正遵守兩規則的＝zoner**＝live 冠軍；`v8:zoner` 逐位元＝量尺 #1 的 `v7:zoner` → 三圖 BT 皆 #1。進攻 `?strategy=hunter` 仍可選。難度 easy/normal/hard ＝BT 強度階梯（hard＝冠軍；DIFFICULTY_PRESET 進攻 sharpness 不變）。完整版本狀態 / 強度 / 評估流程見 **`docs/ai-versions.md`**。
...
- **v6**（凍結，`AI_VERSION = 6`，`client/src/ai/v6/`）＝v5 主幹 ＋ **防守軸再加碼**，破 **v5↔v5 classic 鏡像**。`/goal` 字面的「進攻獵殺」軸（主動逼近／提早獵殺／出農壓制）整條實作後**全對 v5 淨負**（逼近＝送進鏡像封殺彈，再確認控場鏡像天花板）。真正贏法＝防守：`v5-diag` 拆鏡像 38 敗局＝27 中盤封殺＋20 縮圈擠死，全是**逃生分支崩成死胡同**。兩機制疊加：①`entrapWeight` 10→**20**（敵觸發死胡同罰分加重）；②**新增 `shrinkEntrapWeight`（classic 30）**——**敵人無關**、只在縮圈 lead-in 罰死胡同，補 v5 entrap「只在敵近才開、碰不到敵遠縮圈死法」的洞。**classic 直接對 v5 55.8%**（v5-probe 300 局 ~2 SD）、pirate **逐位元＝v5**（兩旋鈕 classic-only）、v3-池 BT 與 v5 並列頂端（1786 vs 1787）無退步。半身閃（lenient hitbox）查證不在 main、被動 port 反稀釋且 pirate 碰不到（死於縮圈非火焰）→ 保留 whole-tile。詳見 `docs/ai-versions.md` §十一。
- **v7**（凍結＝**新 BT 量尺 roster**，`AI_VERSION = 7`，`client/src/ai/v7/`）＝把 v3 那套刻意非遞移的 7-archetype roster 移植到**現行 v6 引擎**、凍結成新量尺（舊 v3 尺早於 PUSH＋移動重做＋三圖 author，在新圖病態失效）。引擎逐位元複製 v6 ＋ 加兩個機制（使用者點子，後由 v8 採用）。**非冠軍**，三圖重 seed `bt-history`，未來版本用 `bt-rank` 上尺。詳見 `docs/ai-versions.md` §十二。
- **v8**（**最新 / live**，`AI_VERSION = 8`，`client/src/ai/v8/`）＝v6 冠軍 roster ＋ 採用 v7 引擎驗證過的**兩條 BnB 三圖戰術規則**：①**「聯通之前不要停止發育」**（孤立時發育 floor 不再隨時鐘 urgency 衰減——打不到被牆隔開的敵人，縮圈遲早逼出聯通，故孤立就發育到完成；`BotController` `isolatedFloor`）；②**「縮圈開始之後，佔據中心」**（`MapProfile.shrinkCenterPriorityWeight`：縮圈真正啟動後中心存活拉力切到大權重、壓過獵殺/封殺，classic+village 20、pirate 0；仍受硬保命閘約束）。兩者皆為 v6→v7 引擎 delta，v8 其餘逐位元＝v6 冠軍 roster。`v8:zoner` 逐位元＝量尺 #1 的 `v7:zoner` → 三圖 BT 皆 #1，且**真正吃到兩規則**（會發育＋會佔中心）＝**live 冠軍**。進攻 `v8:hunter` 是 `pureHunt`（規則1對它無效）、ship-gate 規則2在 classic 害它退步（classic 46.3% / pirate 50.0% / village 65.8%）→ 不當冠軍，僅 `?strategy=hunter` 可選。詳見 `docs/ai-versions.md` §十三。**`v8:zoner` 已在現引擎的純-bot-調參天花板**（2026-06-29 窮舉 ~9 個 lever 全 DROP/wash，且發現**歷史逐旋鈕勝率多已過期**——動 AI 前先用 `v5-screen` 對現 v7 池重量 baseline；別再跑 pocketAvoid／seal-scenario／sealPredict-on-zoner／靜態 corridorGate-on-pirate／純進攻 lead-bomb）。要嚴格超越只剩**改機制**（非對稱縮圈/hitbox）或**人類專用旋鈕**，見 `docs/ai-versions.md` §十四。
- **不做逐 tick golden hash 鎖 AI**：回歸保障由 `determinism.test.ts`（決定性）＋ **`bt-rank`（BT 量尺就位）**＋機制診斷負責。改完活的 AI（v8）後在 `tools/sim-runner/` 跑 `npm run v5-probe -- --target=v8:hunter --opponents=v6:hunter`（直接 ship gate、三圖）＋ `npm run bt-rank -- --target=v8:zoner --map=<圖> --no-write`（量尺定位）＋ `npm test`（含**自陷護欄**已釘 v8 champion）＋ `npm run lint`。caps 若再動須重 `bt-seed` 三圖。
